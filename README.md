...
# 超高精度（用于发表级图表）
energy = np.arange(0.0, 16.0, step=0.05)
```
...
**快速测试** (~2-3分钟，run_quick_test.py 默认):
```python
energy = np.arange(0.0, 16.0, step=0.2)  # 标准网格
N_int_local = 100000                      # 标准样本
```
...
| 目标 | 设置 | 时间 |
|------|------|------|
| 标准计算 (默认) | step=0.2, N=100k, bins=201 | ~2-3分钟 |
| 高精度 | step=0.1, N=500k, bins=201 | ~10分钟 |
| 并行扫描 (8核) | Pool(8) | ~0.3-0.5分钟/点 |
