...
if __name__ == "__main__":
    ...
    U2_values = [1e-5]
    MH_values = [4, 8, 10, 12]
    # U2_values = np.logspace(-6, -1, 6)
    # MH_values = np.linspace(1.0, 15.0, 7)
    ...
    spectrum_nuL_orig = interpolateSpectrum(
        "data/8BSpectrum.csv", energy
    )  # unit: MeV^-1 cm^-2 s^-1

    print(
        "B8 neutrino flux (integrated): ",
        integrateSpectrum(spectrum_nuL_orig),
        "cm^-2 s^-1",
    )
    print()
    ...
    print(
        f">>> Applying {100.0 * energy_resolution:.1f}% energy resolution to background..."
    )
    ...
    print(
        f"Angular resolution: {angle_resolution_deg:.1f}° (for future implementations)"
    )
    ...
    for iu2, U2 in enumerate(tqdm(U2_values, desc="Scan U2", unit="U2", position=0)):
        ...
        for imh, MH, chi2_val, S_bin, signal_centers, sum_S_bin in results:
            if signal_centers.shape != bg_centers.shape or not np.allclose(
                signal_centers, bg_centers, atol=1e-12
            ):
                raise ValueError(
                    "Signal and background energy bins are not aligned for direct chi2"
                )
    ...
    # print(f"Saved chi2 grid to {chi2_npz_path}")
